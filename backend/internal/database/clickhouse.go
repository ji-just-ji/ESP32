...
func (db *ClickHouseDB) SaveSensorReading(reading *models.SensorReading) error {
	ctx := context.Background()

	query := `
		INSERT INTO sensor_readings (timestamp, device_id, temperature, humidity, sound)
		VALUES (?, ?, ?, ?, ?)
	`
	...
	log.Printf("Saved sensor reading to ClickHouse: DeviceID=%s", reading.DeviceID)
	return nil
}
...
func (db *ClickHouseDB) SaveWindowAction(action *models.WindowAction) error {
	...
	query := `
		INSERT INTO window_actions (timestamp, device_id, position, confidence, temperature, humidity, audio_hash)
		VALUES (?, ?, ?, ?, ?, ?, ?)
	`
