...
type WindowAction struct {
	...
	AudioHash   string    `json:"audio_hash"`  // Reference to audio data
}
...
// AudioMetadata contains metadata about audio recordings
type AudioMetadata struct {
	SampleRate int     `json:"sample_rate"` // e.g., 16000 Hz
	Duration   float64 `json:"duration"`    // seconds
}

// SensorReading represents the data received from IoT sensors (legacy, for backward compatibility)
