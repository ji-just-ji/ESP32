...
// DEPRECATED: This file is kept for backward compatibility with the old architecture.
// The new channel-based architecture uses services/inference_service.go instead.
...
func (sa *SensorAggregator) triggerInference(device *DeviceState) {
	...
	// DEPRECATED: This code path is not used in the new architecture
	// Create inference request (using old structure for backward compatibility)
	request := &models.InferenceRequest{
		DeviceID:    device.DeviceID,
		Timestamp:   time.Now(),
		Temperature: device.LastTemperature.Value,
		Humidity:    device.LastHumidity.Value,
		SoundVolume: 0.0, // Would need to be calculated from audio
	}

	log.Printf("DEPRECATED: Triggering inference for %s (temp=%.2f°C, humidity=%.2f%%)",
		device.DeviceID, request.Temperature, request.Humidity)
